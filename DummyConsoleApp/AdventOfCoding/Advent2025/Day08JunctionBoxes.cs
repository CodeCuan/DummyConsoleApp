...
public class Day08JunctionBoxes
{
    ...
    public void Main()
    {
        ...
        stoppy.Stop();
        Console.WriteLine(
            $"Three largest circuits multiplied size: {circuitSize}. Solved in {stoppy.ElapsedMilliseconds} ms"
        );
        stoppy = Stopwatch.StartNew();
        var wallDistance = CheckWallDistance(AdventData2025.Day8JunctionBoxes);
        stoppy.Stop();
        Console.WriteLine(
            $"The wall distance product is: {wallDistance}. Solved in {stoppy.ElapsedMilliseconds} ms"
        );
    }

    public int CheckThreeLargestCircuits(string input, int connectionCount)
    {
        Coordinates = DataParser
            .SplitLines(input)
    ...
    {
        var circuitOne = circuits
            .Where(circuit => circuit.Contains(coordinates.coord1))
            .FirstOrDefault();
        var circuitTwo = circuits
            .Where(circuit => circuit.Contains(coordinates.coord2))
            .FirstOrDefault();
    ...
    private List<LinkedCoordinates3D> GetClosestDistances()
    {
        ...
        foreach (var coordinate in Coordinates)
        {
            ...
            {
                links.Add(new LinkedCoordinates3D(coordinate, neighbour));
            }
        }
