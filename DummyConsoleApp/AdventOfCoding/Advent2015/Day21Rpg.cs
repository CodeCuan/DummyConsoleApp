...
public class Day21Rpg
{
    ...
    public void Main()
    {
        ...
        stoppy.Stop();
        Console.WriteLine(
            $"Minimum cost to win: {bestPlayer.items.Sum(i => i.cost)} .(calculated in {stoppy.ElapsedMilliseconds} ms)"
        );
        Console.WriteLine(
            $"Player had attack {bestPlayer.damage} and def {bestPlayer.armor} with armour set: {string.Join(" | ", bestPlayer.items)}"
        );
        ...
        stoppy.Stop();
        Console.WriteLine(
            $"Maximum cost to lose: {worstPlayer.items.Sum(i => i.cost)} .(calculated in {stoppy.ElapsedMilliseconds} ms)"
        );
        Console.WriteLine(
            $"Player had attack {worstPlayer.damage} and def {worstPlayer.armor} with armour set: {string.Join(" | ", worstPlayer.items)}"
        );
    }
    ...
    private EntityBattler GetMaxCostToLosePlayer(string bossInput)
    {
        SetBoss(bossInput);
        foreach (
            var inputSet in GetItemCombinations().OrderByDescending(items => items.Sum(i => i.cost))
        )
        {
            var player = new EntityBattler(100, inputSet);
            if (!WinsBattle(player))
    ...
    {
        Boss =
            JsonConvert.DeserializeObject<EntityBattler>(input)
