...
public class Day03Routes
{
    public void Main()
    {
        ...
        stoppy.Stop();
        Console.WriteLine(
            $"Houses receiving at least one present: {houses} . Took {stoppy.ElapsedMilliseconds} ms"
        );
        stoppy.Restart();
        var roboHouses = CountHousesWithRobo(AdventData2015.Day3Routes);
        stoppy.Stop();
        Console.WriteLine(
            $"Houses receiving at least one present with Robo-Santa: {roboHouses} . Took {stoppy.ElapsedMilliseconds} ms"
        );
    }
