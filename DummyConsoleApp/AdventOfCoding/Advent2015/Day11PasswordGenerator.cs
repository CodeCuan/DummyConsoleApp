    ...
    public class Day11PasswordGenerator
    {
        public void Main()
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                Console.WriteLine(
                    $"Next valid password: {initialPassword} took {stoppy.ElapsedMilliseconds} ms"
                );
                stoppy.Restart();
            }
        }
        ...
        private static Regex DoubleLetterRegex = new(
            @"([a-z])\1.*([a-z])\2",
        ...
        {
            if (!DoubleLetterRegex.IsMatch(input) || vowelRegex.IsMatch(input))
                return false;
            List<int> letterChain = [];
            foreach (var c in input)
            {
                if (letterChain.Count == 0 || letterChain.Last() + 1 == c)
                {
                    letterChain.Add(c);
