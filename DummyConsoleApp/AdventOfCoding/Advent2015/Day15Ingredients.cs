...
Cinnamon: capacity 2, durability 3, flavor -2, texture -1, calories 3";
    ...
    public void Main()
    {
        ...
        Console.WriteLine(
            $"Sample Score: {GetTotalScore(Sample)} . Took {stoppy.ElapsedMilliseconds} ms"
        );
        stoppy.Restart();
        ...
        stoppy.Restart();
        var totalScoreWithCalories = GetTotalScore(
            AdventData2015.Day15IngredientData,
            caloryReq: 500
        );
        stoppy.Stop();
        Console.WriteLine(
            $"Total Score with 500 calories: {totalScoreWithCalories} in {stoppy.ElapsedMilliseconds}ms"
        );
    }
    ...
    public IEnumerable<long> GetScores(
        List<Ingredient> ingredients,
        int activeIndex = 0,
        int ingredientSpace = 100,
        int? caloryReq = null
    )
    {
    ...
    {
        if (caloryReq.HasValue && ingredients.Sum(i => i.Calories * i.Spoonfuls) != caloryReq.Value)
            return 0;
        List<long> scores =
        [
            ingredients.Sum(i => i.Capacity * i.Spoonfuls),
