...
public class Day08EscapeData
{
    ...
    public void Main()
    {
        ...
        stoppy.Stop();
        Console.WriteLine(
            $"Sample - Total characters of code - characters in memory: {charCount} . Took {stoppy.ElapsedMilliseconds} ms"
        );
        stoppy.Restart();
        var invertedCharCount = EvaluateLinesFromFile(dataFile, true);
        stoppy.Stop();
        Console.WriteLine(
            $"Sample - Total characters to represent escaped strings - original code characters: {invertedCharCount} . Took {stoppy.ElapsedMilliseconds} ms"
        );
    }
    ...
    public int EvaluateLinesFromFile(string file, bool invert)
    {
        ...
        return invert ? lines.Sum(ReverseEvaluateLine) : lines.Sum(EvaluateLine);
    }
    ...
    private static Regex HexadecRegex = new(
        @"\\x[0-9a-f]{2}",
